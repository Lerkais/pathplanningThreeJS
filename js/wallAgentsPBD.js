...
export function clamp2D(vx,vy, maxValue) {
...
export function step(RADIUS,sceneEntities,world) {
  ...
  function sdfStability(agent_i)
  {
    let sdfCell = world.getSDFCell(agent_i.x,agent_i.z);
    if(sdfCell!=undefined)
    {
       //TODO
    }
  }
  ...
  function sdfCollisionConstraint(agent_i)
  {
    let sdfCell = world.getSDFCell(agent_i.px,agent_i.pz);
    if(sdfCell!=undefined)
    {
        //TODO
    }
  ...
  while(pbdIters<ITERNUM)
  {
    ...
    while(i<sceneEntities.length)
    {
      sdfCollisionConstraint(sceneEntities[i]);
      i+=1;
    }
